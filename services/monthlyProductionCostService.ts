/**
 * Monthly Production Cost Service
 * CRUD + calculation for the "monthly_production_costs" collection.
 * Each document is keyed as {productId}_{month} for deterministic upsert.
 */
import {
  collection,
  doc,
...
export const monthlyProductionCostService = {
  ...
  /**
   * Calculate and persist monthly average cost for a product.
   * Uses production_reports filtered by month and the existing cost engine.
   * Will NOT overwrite a month that is already closed.
   */
  async calculate(
    productId: string,
    month: string,
  ...
  /**
   * Recalculate for multiple products in a given month.
   */
  async calculateAll(
    productIds: string[],
    month: string,
  ...
  /**
   * Mark a month as closed so it cannot be recalculated.
   */
  async closeMonth(productId: string, month: string): Promise<void> {
    ...
    const existing = await this.getByProductAndMonth(productId, month);
    if (!existing) return;
    await setDoc(doc(db, COLLECTION, id), { ...existing, isClosed: true, calculatedAt: serverTimestamp() }, { merge: true });
  },

  /**
   * Close a month for ALL products at once.
   */
